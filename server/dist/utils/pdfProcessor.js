...
export async function processPDF(pdfPath, outputDir) {
    ...
    try {
        ...
        for (let i = 0; i < totalPages; i++) {
            ...
            await createPlaceholderImage(path.join(outputDir, imagePath), pageNumber, 1200, 1600);
            await createPlaceholderImage(path.join(outputDir, thumbnailPath), pageNumber, 300, 400);
            pages.push({
                pageNumber,
                imagePath,
                thumbnailPath,
 ...
 */
async function createPlaceholderImage(outputPath, pageNumber, width, height) {
    const svg = `
 ...
 */
export async function deleteDeckFiles(deckDir) {
