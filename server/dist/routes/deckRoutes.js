...
router.get('/:id/analytics', requireUser(), async (req, res) => {
    try {
        const userId = req.user._id.toString();
        const deckId = req.params.id;
...
router.get('/:id/viewers/:viewerId', requireUser(), async (req, res) => {
    try {
        const userId = req.user._id.toString();
        const deckId = req.params.id;
...
// Request: {}
// Response: Image file
router.get('/:id/thumbnails/:pageNumber', async (req, res) => {
    try {
        const deckId = req.params.id;
        const pageNumber = req.params.pageNumber;
        const deck = await deckService.getDeckById(deckId);
