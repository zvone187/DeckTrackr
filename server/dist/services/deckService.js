...
class DeckService {
    ...
    async updateDeck(deckId, userId, data) {
        ...
        if (deck)
            console.log(`[DeckService] Deck updated successfully: ${deckId}`);
        return deck;
    }
    ...
    async getDeckAnalytics(deckId, userId) {
    ...
    async getViewerDetails(deckId, viewerId, userId) {
        ...
        const sessions = await ViewingSession.find({ viewerId, deckId })
            .sort({ startedAt: -1 })
            .lean();
        const sessionsWithSlides = await Promise.all(sessions.map(async (session) => {
